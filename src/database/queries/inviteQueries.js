 ...
 */
export async function saveInviteRole({ inviteCode, roleId, channelId = null, label = null, createdBy = null }) {
  const query = `
    INSERT INTO invite_roles (invite_code, role_id, channel_id, label, created_by)
    VALUES ($1, $2, $3, $4, $5)
  ...
  try {
    const { rows } = await pool.query(query, [inviteCode, roleId, channelId, label, createdBy]);
    return rows[0];
  } catch (error) {
